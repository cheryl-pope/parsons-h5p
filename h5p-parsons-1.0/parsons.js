...
(function($, _) { // wrap in anonymous function to not show some helper variables
        ...
        VariableCheckGrader.prototype._codelinesAsString = function() {
            ...
            $.each(student_code, function(index, item) {
                ...
                for (var i = 0; i < lines.length; i++) {
                    ...
                    executableCode += python_indents[item.indent] + this.parson.$parsonswidget.find("<span>" + lines[i] + "</span>").text() + "\n";
                }
            });
            ...
            if (open && close) { // check blocks only if block definitions are given
                ...
                for (i = 0; i < student_code.length; i++) {
                    var isClose = false, // was a new blocks opened on this line
                        ...
                        item = student_code[i],
                        line = this.parson.$parsonswidget.find("#" + item.id).text(), // code of the line
                        topBlock, bO;
                    ...
                    if (item.indent < 0) {
                        ...
                        this.parson.$parsonswidget.find("#" + item.id).addClass("incorrectIndent");
                    ...
                    for (var blockClose in close) {
                        if (new RegExp(blockClose).test(line)) {
                            ...
                            if (!topBlock) {
                                ...
                                this.parson.$parsonswidget.find("#" + item.id).addClass("incorrectPosition");
                ...
                for (i = 0; i < blocks.length; i++) {
                    ...
                    this.parson.$parsonswidget.find("#" + blocks[i].item.id).addClass("incorrectPosition");
                }
            }
        ...
        var LineBasedGrader = function(parson) {
            this.parson = parson;
            console.log("this is a parsonswidget created in LinbasedGrader");
        };
        ...
        LineBasedGrader.prototype.grade = function(elementId) {
            ...
            if (parson.model_solution.length < student_code.length) {
                this.parson.$parsonswidget.find("#ul-" + elemId).addClass("incorrect");
        ...
        ParsonsCodeline.prototype.elem = function() {
            ...
            return this.widget.$parsonswidget.find("#" + this.id);
        };
        ...
        ParsonsWidget.prototype.getHash = function(searchString) {
            var hash = [],
                ids = this.$parsonswidget.find(searchString).sortable('toArray'),
                line;
        ...
        ParsonsWidget.prototype.getModifiedCode = function(search_string) {
            ...
            var lines_to_return = [],
                solution_ids = this.$parsonswidget.find(search_string).sortable('toArray'),
                i, item;
        ...
        ParsonsWidget.prototype.updateHTMLIndent = function(codelineID) {
            ...
            this.$parsonswidget.find('#' + codelineID).css("margin-left", this.options.x_indent * line.indent + "px");
        };
        ...
        ParsonsWidget.prototype.createHTMLFromLists = function(solutionIDs, trashIDs) {
            ...
            var that = this;
            var sortable = this.$parsonswidget.find("#ul-" + this.options.sortableId).sortable({
